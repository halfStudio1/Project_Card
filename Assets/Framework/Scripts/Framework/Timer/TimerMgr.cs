...
public class TimerMgr : Singleton<TimerMgr>
{
    ...
    IEnumerator StartTiming(bool isRealTime, Dictionary<int, TimerItem> timerDic)
    {
        while (true)
        {
            ...
            for (int i = 0; i < delList.Count; i++)
            {
                ...
                //放入缓存池中
                ObjectPoolMgr.Instance.PushObj(delList[i]);
            }
            //移除结束后 清空列表
            delList.Clear();
        }
    ...
    public int CreateTimer(bool isRealTime, int allTime, UnityAction overCallBack, int intervalTime = 0, UnityAction callBack = null)
    {
        ...
        int keyID = ++TIMER_KEY;
        //从缓存池取出对应的计时器
        TimerItem timerItem = ObjectPoolMgr.Instance.GetObj<TimerItem>();
        //初始化数据
    ...
    public void RemoveTimer(int keyID)
    {
        ...
        {
            //移除对应id计时器 放入缓存池
            ObjectPoolMgr.Instance.PushObj(timerDic[keyID]);
            //从字典中移除
        ...
        {
            //移除对应id计时器 放入缓存池
            ObjectPoolMgr.Instance.PushObj(realTimerDic[keyID]);
            //从字典中移除
